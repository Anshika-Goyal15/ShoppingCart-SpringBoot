...
const Product = () => {
  ...
  const retrieveProduct = async () => {
    const response = await axios.get(
      "http://localhost:5050/api/product/id?productId=" + productId
    );
  ...
  const retrieveProductsByCategory = async () => {
    const response = await axios.get(
      "http://localhost:5050/api/product/category?categoryId=" + categoryId
    );
  ...
  const saveProductToCart = (userId) => {
    fetch("http://localhost:5050/api/user/cart/add", {
      method: "POST",
      headers: {
  ...
  const addToCart = (e) => {
    if (user == null) {
      ...
      if(product.quantity === 0){
        ...
        alert(`Only ${product.quantity} units are left!!`);
      }
     else {
      ...
      <div class="row">
        ...
        <div class="col-sm-3 mt-2 admin">
          <div class="card form-card border-color custom-bg">
            <img
              src={"http://localhost:5050/api/product/" + product.imageName}
              style={{
                maxHeight: "500px",
                maxWidth: "100%",
                width: "auto",
